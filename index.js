...
app.post("/add-recipe", async (req, res) => {
  ...
  const { authorEmail } = newRecipe;
  // console.log("Author Email:", authorEmail); // Debugging line

  const userInfo = await userCollection.findOne({ email: authorEmail });
  const isPremium = userInfo?.isPremium || false;
  const recipeCount = await recipesCollection.countDocuments({ authorEmail });

  if (!isPremium && recipeCount >= 2) {
    return res.status(403).send({
      limitReached: true,
      message:
        "ফ্রি প্ল্যানে সর্বোচ্চ ২টি রেসিপি অ্যাড করা যায়। আরও রেসিপি অ্যাড করতে প্রিমিয়াম প্ল্যান কিনুন।",
    });
  }
...
app.post("/add-featured-recipes", async (req, res) => {
  try {
    ...
    const result = await addFeaturedRecipeCollection.insertOne(featuredRecipe);
